...
age  <- age + 20        # age? 41
mass_index <- mass/age # mass_index? 3.98048780487805
```
...
It’s good practice to put the non-optional arguments (like the number you’re rounding) first in your function call, and then specify the names of all optional arguments. If you don’t, someone reading your code (including future you) might have to look up the definition of a function with unfamiliar arguments to understand what you’re doing.
...
# Why do you think it happens? It will only be classified as a character when there are quotation marks in the list. Without quotes, the values will be registered as numerical.
```
...
First it looks for numeric values in the vector, then non-numerical values. By default it would classify to character.

```{r}
# Build a vector of the study semesters: 
stdy_semesters <- c("Winter_2015", "Fall_2015", "Winter_2016", "Fall_2016", "Winter_2017", "Fall_2017", "Winter_2018", "Fall_2018", "Winter_2019", "Fall_2019")

# Build a vector where the supplements consumed matches the order of the study semesters:  
supplements_per_semester <- c("RPS", "RMS, RPS, Inulin, Arabinoxylan", "RPS, psyllium", "RPS", "RMS, Inulin, Accessible", "RPS, HiMaze, banana", "HiMaze", "RPS, RMS", "RPS", "RPS, dietary fibers (oat, rice, potato)")
...
age_college_students <- c(18, 19, 18, 18, 20, 18, 19, 21, 20)
# Confirm this is numeric
class(age_college_students)
# Convert this vector to character:
age_college_students <- c("18", "19", "18", "18", "20", "18", "17", "21", "20")
```
