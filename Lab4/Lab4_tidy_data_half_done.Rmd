...
```{r}
scfa_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, butyrate_mean_mmol_kg) %>% #keep only columns of interest
  filter(study_week == "week1" | study_week == "week3") %>% #keep only weeks of interest
  spread(study_week, butyrate_mean_mmol_kg) #widen data frame
```
...
```{r}
butyrate_wide <- scfa_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, butyrate_mean_mmol_kg) %>% #drop extra columns
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(study_week, butyrate_mean_mmol_kg) %>% #convert to wide format
...
acetate_wide <- scfa_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, acetate_mean_mmol_kg) %>% #drop extra columns
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(study_week, acetate_mean_mmol_kg) %>% #convert to wide format
  na.omit(week1, week3) %>% #drop NAs from select columns
...
propionate_wide <- scfa_qc %>%
  select(participant_id, semester, supplement_consumed, study_week, propionate_mean_mmol_kg) %>% #drop extra columns
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(study_week, propionate_mean_mmol_kg) %>% #convert to wide format
  na.omit(week1, week3) %>% #drop NAs from select columns
