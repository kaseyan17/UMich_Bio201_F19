...
```{r}
scfa_data <- read_delim(file = "Lab3/scfa_indv_qc.txt",
...
scfa_data %>%
  ...
  group_by(semester, supplement_consumed) %>%
  summarize(mean_butyrate = mean(butyrate_mmol_kg, na.rm = TRUE)) #na remove function 
```
...
scfa_data %>%
  ...
  summarize(mean_butyrate = mean(butyrate_mmol_kg), 
            sd_butyrate = sd(butyrate_mmol_kg), 
            n_butyrate = n()) %>%
  arrange(-mean_butyrate) #drop "-" to put lowest last 
```
...
```{r}
# write code here
scfa_indv_summary <- scfa_data %>%
  filter(!is.na(butyrate_mmol_kg)) %>%
  group_by(participant_id, study_week) %>%
  summarize(mean_butyrate = mean(butyrate_mmol_kg), 
            mean_acetate = mean(acetate_mmol_kg), 
            mean_propionate = mean(propionate_mmol_kg)) %>%
  arrange(-mean_butyrate, -mean_acetate, -mean_propionate)
...
```{r}
scfa_wkly_qc%>%
  select(participant_id, semester, study_week, butyrate_mean) %>% #keep only columns of interest
...
delta_butyrate_wide <- butyrate_wide %>%
  ...
  summarize(mean_butyrate_delta = mean(delta_butyrate))
...
delta_acetate_wide <- acetate_wide %>%
  group_by(supplement_consumed) %>%
  summarize(mean_acetate_delta = mean(delta_acetate))
```
...
delta_propionate_wide <- propionate_wide %>%
  group_by(supplement_consumed) %>%
  summarize(mean_propionate_delta = mean(delta_propionate))
```

```{r}
# copy+paste and edit code chunks for plots
plot1 <- ggplot(data = butyrate_wide, 
       mapping = aes(x = supplement_consumed, 
                     y = delta_butyrate))
plot2 <- ggplot(data = acetate_wide,
                mapping = aes(x = supplement_consumed,
                              y = delta_acetate))
plot3 <- ggplot(data = propionate_wide,
                mapping = aes(x = supplement_consumed,
                              y = delta_propionate))
# Draw the plot
plot1 + geom_point(color = 'darkred')
plot2 + geom_point(color = 'darkblue')
plot3 + geom_point(color = 'darkgreen')
...
```{r}
nrow(acetate_wide) #rows in data frame 457
n_distinct(acetate_wide$participant_id) #number of participants, should equal number rows 457

nrow(butyrate_wide) #422
n_distinct(butyrate_wide$participant_id) #422

nrow(propionate_wide) #361
n_distinct(propionate_wide$participant_id) #361
```
...
join3 <- inner_join(acetate_wide, butyrate_wide,
  ...
  full_join(propionate_wide, 
            by = c("participant_id", "semester", "supplement_consumed")) #inner only roduces the matches and full brings in all the non matches with NAs
...
```{r}
# write code here
total_scfas <-full_join(acetate_wide, propionate_wide,
            by = c("participant_id", "semester", "supplement_consumed")) %>% 
  full_join(butyrate_wide,
            by = c("participant_id", "semester", "supplement_consumed")) %>% 
  mutate(week1_total = ace_wk1 + but_wk1 + pro_wk1,
         week3_total = ace_wk3 + but_wk3 + pro_wk3,
         delta_total_week = week3_total - week1_total)
``` 
...
```{r}
# write draft of plot code here, the real thing is above in one long pipe...
total_scfas_plot <-  ggplot(data = total_scfas <-full_join(acetate_wide, propionate_wide,
            by = c("participant_id", "semester", "supplement_consumed")) %>% 
  full_join(butyrate_wide,
            by = c("participant_id", "semester", "supplement_consumed")) %>% 
  mutate(week1_total = ace_wk1 + but_wk1 + pro_wk1,
         week3_total = ace_wk3 + but_wk3 + pro_wk3,
         delta_total_week = week3_total - week1_total), 
       mapping = aes(x = delta_total_week, 
                     y = supplement_consumed, semester, 
                     color = supplement_consumed)) + 
  geom_point() + 
  labs(title = "Total SCFA for Each Supplement ",
         x = "Delta Total Week",
         y = "Supplement") 
total_scfas_plot #I chose to do two full_join functions in order to keep all the columns even if they contain N/A values
```
...
total_scfas %>%
  ...
  gather(key = SCFA_wk, value = SCFA_concentration,
         ace_wk1, ace_wk3, but_wk1, but_wk3, pro_wk1, pro_wk3, week1_total, week3_total)
```
...
ace_long <- total_scfas %>%
  ...
  select(-starts_with("delta_"), -starts_with("but_"), -starts_with("pro_"), -ends_with("_total")) %>% 
...
but_long <-total_scfas %>%
  #drop diff columns & extra SCFAs
  select(-starts_with("delta_"), -starts_with("ace_"), -starts_with("pro_"), -ends_with("_total")) %>% 
...
pro_long <- total_scfas %>%
  #drop diff columns & extra SCFAs
  select(-starts_with("delta_"), -starts_with("but_"), -starts_with("ace_"), -ends_with("_total")) %>% 
...
```{r}
scfa_long <- full_join(pro_long, ace_long,
...
wkly_means <- scfa_long %>%
  group_by(study_week, scfa, supplement_consumed, semester) %>% 
  summarize(mean_scfa_total = mean(concentration_mmol_kg, na.rm = TRUE))
```
...
p1 <- wkly_means %>%
  ...
  ggplot(aes(x = semester, y = mean_scfa_total, scfa)) +
...
write_delim(join3, path = "Lab4/curated_data/join3.txt", delim = "\t")
write_delim(wkly_means, path = "Lab4/curated_data/wkly_means.txt", delim = "\t")

# for plots:
save_plot(filename = "p1.pdf",
          plot = p1,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
save_plot(filename = "total_scfas_plot.pdf",
          plot = total_scfas_plot,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
save_plot(filename = "plot1.pdf",
          plot = plot1,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
save_plot(filename = "plot2.pdf",
          plot = plot2,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
save_plot(filename = "plot3.pdf",
          plot = plot3,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
```
...
pH_qc <- pH_data %>%
  ...
  summarise(wkly_av_pH = mean(ph, na.rm = TRUE)) #calculate weekly average = 739 rows, 4 columns
dim(pH_qc)
```
...
BSS_qc <- BSS_data %>%
  ...
  group_by(participant_id, study_week, supplement_consumed) %>%
  summarise(wkly_av_bristol = mean(bristol_numeric, na.rm = TRUE)) #calculate weekly average = 411 rows and 4 columns
dim(BSS_qc)
```

Select the appropriate type of join to retain participants with either pH or BSS measurements. 
```{r}
pH_BSS_wkly <- full_join(BSS_qc, pH_qc,
                    by = c("participant_id", "study_week", "supplement_consumed")) %>%
  mutate(rounded_ph = round(wkly_av_pH, digits = 2),
         rounded_brist = round(wkly_av_bristol, digits = 2)) %>% 
  select(-wkly_av_pH, -wkly_av_bristol) #round avgs to reflect sig figs
```
...
```{r}
write_delim(pH_BSS_wkly, path = "Lab4/curated_data/pH_BSS_wkly.txt", delim = "\t")
```
