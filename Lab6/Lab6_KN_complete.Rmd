...
knitr::opts_knit$set(root.dir = "~/Documents/UMich_Bio201_F19_new/")
```
...
tax_scfa <- inner_join(scfa_long, taxa_cat_curated,
                 by = c("study_week", "participant_id", "semester"))
```
...
# sample size of each group. They are huge (greater than a thousand).
tax_scfa %>% 
...
Here we have a small p-value, this means at least one of our groups is significantly different from the others. To identify which group is different we have to run a post-hoc test (an additional test). We will use Tukey's Honestly Significant Difference (HSD).
...
write_delim(tuk_wk1, path = "~/Documents/UMich_Bio201_F19_new/Lab6/curated_data/tuk_wk1.txt",
...
```{r}
# import methane data 
breath_wkly <- read_delim("~/Documents/UMich_Bio201_F19_new/Lab5/curated_data/breath_wkly.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA")) %>%
# format methane data
  rename_all(tolower) %>%
  ...
  mutate(semester = factor(semester,
                           levels=c("Winter2015", "Fall2015", 
                                    ...
                                    "Winter2019"), ordered = TRUE))
# plot
brth_wkly_violin_plot <- breath_wkly %>%
...
# run the test and saved
kruskal.test(supplement_consumed ~ ch4_mean, breath_wkly)
#p-value = 0.1579
save_plot(filename = "brth_wkly_violin_plot.pdf",
          plot = brth_wkly_violin_plot,
          nrow = 1, ncol = 2, 
...
```{r}
# hypothesis: 
#Null: Taking fiber supplements doesn't affect hydrogen levels.
#Alternative: Taking fiber supplements affects the hydrogen levels.

# plot 
h2_violin_plot <- breath_wkly %>%
  ggplot(aes(x = supplement_consumed, 
             y = h2_mean, 
             color = supplement_consumed)) + 
  geom_violin() + 
...
save_plot(filename = "h2_violin_plot.pdf",
          plot = h2_violin_plot,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
```
...
```{r}
# sample size of each group. They are huge (greater than a thousand). The BRMPS is over 5,000 (n=9730) 
tax_scfa %>% 
...
ggplot(s1, aes(x = h2_mean)) + geom_histogram()

s2 <- breath_wkly %>%
  filter(supplement_consumed == "Arabino") 
shapiro.test(s2$h2_mean) #p-value = 0.1695
ggplot(s2, aes(x = h2_mean)) + geom_histogram()

s3 <- breath_wkly %>% 
  filter(supplement_consumed == "BRMPS")
shapiro.test(s3$h2_mean) #p-value = 2.823e-12
ggplot(s3, aes(x = h2_mean)) + geom_histogram()

s4 <- breath_wkly %>%
  filter(supplement_consumed == "BRMPS+Accessible")
shapiro.test(s4$h2_mean) #p-value = 0.05314
ggplot(s4, aes(x = h2_mean)) + geom_histogram()

s5 <- breath_wkly %>%
  filter(supplement_consumed == "HiMaize") 
shapiro.test(s5$h2_mean) #p-value = 9.956e-06
ggplot(s5, aes(x = h2_mean)) + geom_histogram()

s6 <- breath_wkly %>%
  filter(supplement_consumed == "HiMaize+BRMPS") 
shapiro.test(s6$h2_mean) #p-value = 0.001864
ggplot(s6, aes(x = h2_mean)) + geom_histogram()

s7 <- breath_wkly %>%
  filter(supplement_consumed == "Inulin") 
shapiro.test(s7$h2_mean) #p-value = 0.0002929
ggplot(s7, aes(x = h2_mean)) + geom_histogram()

s8 <- breath_wkly %>% 
  filter(supplement_consumed == "LOODAT")
shapiro.test(s8$h2_mean) #p-value = 7.089e-05
ggplot(s8, aes(x = h2_mean)) + geom_histogram()

s9 <- breath_wkly %>% 
  filter(supplement_consumed == "Psyllium")
shapiro.test(s9$h2_mean) #p-value = 0.4093
ggplot(s9, aes(x = h2_mean)) + geom_histogram()

s10 <- breath_wkly %>% 
  filter(supplement_consumed == "Psyllium+BRMPS")
shapiro.test(s10$h2_mean) #p-value = 0.472
ggplot(s10, aes(x = h2_mean)) + geom_histogram()

s11 <- breath_wkly %>% 
  filter(supplement_consumed == "transition_HiMaize")
shapiro.test(s11$h2_mean) #p-value = 7.102e-07
ggplot(s11, aes(x = h2_mean)) + geom_histogram()
...
#p-value = 0.3001, fail to reject null.
```

* What is the answer to the question? Write your conclusions:
```{r}
# conclusion: Fail to reject the null hypothesis (p-value=0.3001). Taking fiber supplements doesn't affect the hydrogen levels.
```
...
taxratio_plot <- tax_ratio %>%
  ...
  labs(title = "Firmicutes abundance versus butyrate concentration")
taxratio_plot

save_plot(filename = "tax_ratio_correlation_plot.pdf",
          plot = taxratio_plot,
          nrow = 1, ncol = 1,
          base_aspect_ratio = 1.1)
...
summary(m63a) #view results P-value = < 2.2e-16 and Adjusted R-squared:  0.003863. The p-value is significant, the R-quared is not... 
```

Do you draw the same conclusions if you use the butyrate ratio and Firmicutes to Bacteroidetes ratio (f_b_ratio)?
```{r}
# plot
but_fb_ratio_plot <- tax_ratio %>%
  ...
  geom_smooth(method = "lm", 
              se = FALSE) + 
  xlab("Butyrate ratio") + 
  ylab("Ratio of Firmicutes to bacteroidetes") +
  labs(title = "Butyrate ratio to the ratio of Firmicutes to bacteroidetes")
but_fb_ratio_plot

save_plot(filename = "but_fb_ratio_correlation_plot.pdf",
          plot = but_fb_ratio_plot,
          nrow = 1, ncol = 1,
          base_aspect_ratio = 1.1)
...
summary(m63b) #P-value = < 2.2e-16, very significant
#Adjusted R-squared:  0.02854, not significant
```
...
```{r}
prop_bacteriodetes_ratio_plot <- tax_ratio %>%
  ...
  labs(title = "Bacteroidetes abundance versus propionate concentration")
prop_bacteriodetes_ratio_plot

save_plot(filename = "prop_bacteriodetes_ratio_plot.pdf",
          plot = prop_bacteriodetes_ratio_plot,
          nrow = 1, ncol = 1,
          base_aspect_ratio = 1.1)

# correlation test 
correlation_prop_bact_ratio <- tax_ratio %>%
  lm(propionate_mean ~ bacteroidetes_abund, data = .) 
summary(correlation_prop_bact_ratio) #view results P-value = < 2.2e-16, very significant
#Adjusted R-squared:  0.04197, not significant
```

Do you draw the same conclusions if you use the propionate ratio and Firmicutes to Bacteroidetes ratio?
```{r}
prop_f_b_ratio_plot <- tax_ratio %>%
  ...
  labs(title = "Ratio or propionate to rate of firmicutes to bacteroidetes")
prop_f_b_ratio_plot

save_plot(filename = "prop_f_b_ratio_plot.pdf",
          plot = prop_f_b_ratio_plot,
          nrow = 1, ncol = 1,
          base_aspect_ratio = 1.1)

# correlation test 
correlation_prop_f_b_ratio <- tax_ratio %>%
  lm(propionate_ratio ~ f_b_ratio, data = .) 
summary(correlation_prop_f_b_ratio) 
#P-value = < 2.2e-16, I got the same results (p-value and insign. R-squared) except the correlation is now negative.
```
...
```{r}
# import 
pH_BSS_wkly <- read_delim("Lab4/curated_data/pH_BSS_wkly.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA"))
...
```{r}
SCFA_total_acetate_plot <- tax_scfa_pH %>%
  ggplot(aes(x = relative_abundance,
             y = acetate_mean)) + 
  geom_point() + 
  geom_smooth(method = "lm", 
              se = FALSE) + 
  xlab("Relative total SCFA abundance (mmol/kg)") + 
  ylab("Mean of acetate") +
  labs(title = "Relative abundance of SCFAs to total acetate levels")
SCFA_total_acetate_ratio_plot

save_plot(filename = "SCFA_total_acetate_plot.pdf",
          plot = SCFA_total_acetate_plot,
          nrow = 1, ncol = 1,
          base_aspect_ratio = 1.1)

# correlation test 
correlation_SCFA_total_acetate <- tax_scfa_pH %>%
  lm(relative_abundance ~ acetate_mean, data = .) 
summary(correlation_SCFA_total_acetate) #view results P-value = 0.6549, not significant & Adjusted R-squared = -0.0001349, no results. There is no relationship.
```

Do you draw the same conclusions if you use the total SCFAs and acetate ratio?
```{r}
SCFA_total_acetate_ratio_plot <- tax_scfa_pH %>%
  ggplot(aes(x = relative_abundance,
             y = acetate_ratio)) + 
  geom_point() + 
  geom_smooth(method = "lm", 
  ...
  xlab("Relative total SCFA abundance (mmol/kg)") + 
  ylab("Acetate ratio") +
  labs(title = "Correlation between relative abundance of SCFAs to ratio acetate")
SCFA_total_acetate_ratio_plot

save_plot(filename = "SCFA_total_acetate_ratio_plot.pdf",
          plot = SCFA_total_acetate_ratio_plot,
          nrow = 1, ncol = 1,
          base_aspect_ratio = 1.1)

# correlation test 
correlation_SCFA_total_acetate_ratio <- tax_scfa_pH %>%
  lm(relative_abundance ~ acetate_ratio, data = .) 
summary(correlation_SCFA_total_acetate_ratio) #view results P-value = 0.1988, not significant & Adjusted R-squared = 0.0001098, very low correlation, not a signicicant difference. There is no relationship. 
```
...
```{r}
SCFA_total_pH_plot <- tax_scfa_pH %>%
  ggplot(aes(x = relative_abundance,
             y = rounded_ph)) + 
  geom_point() + 
  geom_smooth(method = "lm", 
...
save_plot(filename = "SCFA_total_pH_plot.pdf",
          plot = SCFA_total_pH_plot,
          nrow = 1, ncol = 1,
          base_aspect_ratio = 1.1)

# correlation test 
correlation_SCFA_pH <- tax_scfa_pH %>%
  lm(relative_abundance ~ acetate_ratio, data = .) 
summary(correlation_SCFA_pH) #view results P-value = p-value: 0.1988, not significant & Adjusted R-squared = 0.0001098, very low correlation, no significance. There is no relationship.
```
...
butyrate_pH_plot <- tax_scfa_pH %>%
  ...
  xlab("Relative total SCFA abundance (mmol/kg)") + 
  ylab("pH levels") +
  labs(title = "Correlation between mean butyrate and the pH levels")
butyrate_pH_plot

save_plot(filename = "butyrate_pH_plot.pdf",
          plot = butyrate_pH_plot,
          nrow = 1, ncol = 1,
          base_aspect_ratio = 1.1)

# correlation test 
correlation_butyrate_pH <- tax_scfa_pH %>%
  lm(butyrate_mean ~ rounded_ph, data = .) 
summary(correlation_butyrate_pH) #view results P-value = p-value: < 2.2e-16, very significant & Adjusted R-squared = 0.05209, very low correlation, no significance. There is a slight relationship ince the p-value proofsa significance but the R-squared is very low.
```
...
```{r}
write_delim(taxa_cat_curated, path = "Lab6/curated_data/taxa_cat_curated.txt", delim = "\t")
write_delim(scfa_ratio, path = "Lab6/curated_data/scfa_ratio.txt", delim = "\t")
write_delim(tax_scfa, path = "Lab6/curated_data/tax_scfa.txt", delim = "\t")
write_delim(tax_scfa_pH, path = "Lab6/curated_data/tax_scfa_pH.txt", delim = "\t")
```
