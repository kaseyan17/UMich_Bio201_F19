...
write_delim(genus_long, 
            path = "~/Documents/UMich_Bio201_F19/Lab8/curated_data/genus_long.txt",
...
save_plot(filename = "genus_plot.pdf",
          plot = genus_plot,
          nrow = 1, ncol = 1, 
...
write_delim(order_long, 
            path = "~/Documents/UMich_Bio201_F19/Lab8/curated_data/order_long.txt",
            delim = "\t", na = "NA", col_names = TRUE)
...
save_plot(filename = "order_plot.pdf",
          plot = order_plot,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 5.5)
```
...
physq_j <- ordinate(physq_sub, 
                      ...
                      distance = "jaccard")
physq_j
#stress value = 0.2324081, pretty low but not lower than 0.20...
```
...
```{r}
jaccard_plot1 <- plot_ordination(physeq = physq_sub, 
                     ordination = physq_j, 
                     type = "samples", 
                     color = "semester", 
                     shape = "study_week")
...
save_plot(filename = "jaccard_plot1.pdf",
          plot = jaccard_plot1,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)

save_plot(filename = "jaccard_plot2.pdf",
          plot = jaccard_plot2,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 1.1)
```
...
jaccard_df <- physq_sub %>% 
  sample_data(.) %>% 
  as(., "data.frame")  

# run test
jaccard_adn_results <- adonis(formula = dat_jaccard ~ semester * study_week, 
                  data = jaccard_df)

print(jaccard_adn_results)
```
p-values: semester=0.001, study_week=0.001, & semester:study_week 0.973 
R2: semester=0.02871, study_week=0.00774, & semester:study_week 0.00367

There is a significance when grouping by semester or by study_week according to the calculated p-values (both 0.001). The R2 values of 0.02871 and 0.00774 are fairly close to 0, meaning no significant dissimilarity between the different groupings. Since both values are low, the variation in the groupings is likely to influence the resulting data.
