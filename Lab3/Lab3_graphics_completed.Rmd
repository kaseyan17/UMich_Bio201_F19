...
knitr::opts_knit$set(root.dir = "~/Documents/UMich_Bio201_F19/Lab3/raw_data")
```
...
scfa_qc <- read_delim(file = "SCFA_wkly.txt", 
                        delim = "\t", escape_double = FALSE, trim_ws = TRUE, na=c("NA"),
                        col_types = list()) %>% 
  select(-Butyrate_median, -Acetate_median, -Propionate_median) %>% 
  filter(Quantity_compliant == "yes") %>% 
  rename_all(tolower) %>% 
  rename(butyrate_mean_mmol_kg = butyrate_mean,
         acetate_mean_mmol_kg = acetate_mean, 
         propionate_mean_mmol_kg = propionate_mean) %>% 
  mutate(total_scfa_mmol_kg = butyrate_mean_mmol_kg + acetate_mean_mmol_kg + propionate_mean_mmol_kg)
...
plot_tot <- ggplot(data = scfa_qc, 
    ...
    labs(title = "Total Mean Concentrations of Individuals",
         x = "Individuals",
         y = "Fecal total (mmol/kg)") +
    theme(axis.text.x = element_blank())
plot_tot
```
...
plot_f <- scfa_qc %>%
  ...
  labs(title = "Total Fecal Short Chain Fatty Acid for Individuals", x = "Individuals",
       y = "Total fecal SCFAs (mmol/kg)") +
  theme(axis.text.x = element_blank(),
...
> ** I added a legend so it will be easier to read, and I made atitle to summarize what is going on in the graph... ** 
...
```{r}
but_plot_final <- scfa_qc %>%
  filter(study_week == "week3") %>%
  ggplot(aes(x = semester, 
             y = butyrate_mean_mmol_kg)) +
  geom_point(aes(color = supplement_consumed)) +
  facet_grid(~supplement_consumed) + 
  labs(title = "Mean Butyrate Concentrations Over Three Weeks of the Semester", 
       x = "Semester",
       y = "Butyrate fecal (mmol/kg)") +
  theme(axis.text.x = element_blank(),
...
Export plot 3.6 
```{r}
save_plot(filename = "but_plot_final.pdf",
          plot = but_plot_final,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
