...
colnames(scfa_indv) #"Participant_ID", "Sex", "Age", "Race/ethnicity", "Ht_in", and 14 more...
rownames(scfa_indv) #"1", "2", ..., "1000"
glimpse(scfa_indv) #shows what the variables look like next to some of the observations, not very easy to read though.
str(scfa_indv) #shows what the variables look like next to the observations yet again, this time a little easier to read.
```
...
```{r eval=FALSE, include=FALSE}
select(scfa_indv) #subset columns 4078 rows
filter(scfa_indv, semester == "Fall2018") #subset rows on conditions
...
```{r}
select(scfa_indv, study_week, participant_id)
```
...
scfa_indv %>%
  ...
  select(participant_id, study_week, supplement_consumed, notes)
```
...
scfa_indv_qc <- scfa_indv %>%
  rename_all(tolower) %>% 
  mutate(wt_kg = round((wt_lbs * 2.205), digits = 0)) %>% 
  mutate(ht_in = round((ht_in * 35.274), digits = 0)) %>% 
  filter(sample_wt_g > 0.10, 
         notes != "frozen >24hrs") %>% 
  mutate(sample_wt_kg = sample_wt_g / 1000) %>% 
  mutate(acetate_mmol_kg = (acetate_mm*0.002)/sample_wt_kg,
         butyrate_mmol_kg = (butyrate_mm*0.002)/sample_wt_kg,
         propionate_mmol_kg = (propionate_mm*0.002)/sample_wt_kg) %>% 
  mutate(total_scfa_mm = round((acetate_mm + butyrate_mm + propionate_mm), digits = 2)) %>% 
  select(-ht_in, -wt_lbs, -acetate_mm, -butyrate_mm, -propionate_mm)
```

Swap your code with your neighbor, does their code run on your computer? Are the outputs the same? Didn't have time
...
scfa_wkly_qc <- read_excel("raw_data/SCFA_data.xlsx", 
  ...
  mutate(total_mean_scfa = acetate_mean + butyrate_mean + propionate_mean) %>% 
  mutate(total_median_scfa = acetate_median + butyrate_median + propionate_median)
```
